#use accelerated proximal gradient descent to solve LASSO formulation, which is convex 
import math
import random
import numpy as np
...
def prox_operation(lmd,step_length,w):
    new_w = sign(w) 
    print 'new_w',new_w
    prox_vec = new_w * vec_max(0, np.absolute(w) - step_length*lmd)   # NOTE: here is step_length * lmd 
    print prox_vec
    return prox_vec
#--------------------------------------------------------------------------------    
# gradient step
def gradient_descent(y_k, step_length,A,b):
    f_prime = np.dot(A.T, np.dot(A,y_k.T) - b) # derivertive of f_function 
    print 'f_prime',f_prime
    w = y_k - step_length * f_prime
    return w 
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        while(1):
            print 'x_k',x_k
            w = gradient_descent(y_k, step_length, A, b)
            print 'w',w
            x_k_plus_1 = prox_operation(lmd,step_length, w)
            ...
            print '********************************************'
        t_k_plus_1 = 0.5 + 0.5*math.sqrt(1+4*math.pow(t_k,2))
